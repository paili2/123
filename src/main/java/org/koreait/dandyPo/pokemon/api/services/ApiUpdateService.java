package org.koreait.dandyPo.pokemon.api.services;




import lombok.RequiredArgsConstructor;
import org.koreait.dandyPo.pokemon.api.entities.ApiPokemon;
import org.koreait.dandyPo.pokemon.api.entities.ApiResponse;
import org.koreait.dandyPo.pokemon.api.entities.UrlItem;
import org.koreait.dandyPo.pokemon.entities.Pokemon;
import org.koreait.dandyPo.pokemon.repositories.PokemonRepository;
...
public class ApiUpdateService {
    ...
    public void update(int page) {
        ...
        for (UrlItem item : items) {
            ...
            pokemon.setName(data1.getName()); // 영문 이름
            ...
            String types = data1.getTypes().stream().map(d -> d.getType().getName())
                    .collect(Collectors.joining("||")); // 타입1||타입2||타입3 형태
            /* 타입 처리 E */
            ...
            // 한글 이름
            String nameKr = data2.getNames().stream().filter(d -> d.getLanguage().getName().equals("ko"))
                    .map(d -> d.getName()).collect(Collectors.joining());
            pokemon.setName(nameKr);

            // 한글 설명
            String flavorText = data2.getFlavorTextEntries().stream().filter(d -> d.getLanguage().getName().equals("ko"))
                    .map(d -> d.getFlavorText()).collect(Collectors.joining());
